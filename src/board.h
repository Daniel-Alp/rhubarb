#pragma once

#include "common.h"
#include "types.h"
#include <array>
#include <string>

extern std::string start_fen;

enum Square : u32 {
	A1, B1, C1, D1, E1, F1, G1, H1,
	A2, B2, C2, D2, E2, F2, G2, H2,
	A3, B3, C3, D3, E3, F3, G3, H3,
	A4, B4, C4, D4, E4, F4, G4, H4,
	A5, B5, C5, D5, E5, F5, G5, H5,
	A6, B6, C6, D6, E6, F6, G6, H6,
	A7, B7, C7, D7, E7, F7, G7, H7,
	A8, B8, C8, D8, E8, F8, G8, H8, NO_SQ
};

struct Undo {
	Square en_passant_sq;
	u32 castling_rights;
	u32 fifty_move_rule;
	u64 zobrist_key;
};

struct Position {
	Color side_to_move;

	std::array<Piece, 64> pces;
	std::array<u64, 15> pce_bitboards;
	std::array<u64, 2> col_bitboards;
	u64 all_bitboard;

	u32 castling_rights;
	Square en_passant_sq;
	i32 fifty_move_rule;
	u64 zobrist_key;

	i32 ply;
	i32 history_ply;
	std::array<Undo, 256> undo_stack;
	std::array<u64, 256> history_stack;

	i32 phase_val;
	i32 material_midgame_val;
	i32 material_endgame_val;
	i32 psqt_midgame_val;
	i32 psqt_endgame_val;
};

Position load_from_fen(const std::string &fen);
inline i32 get_rank(const i32 sq) {
	return sq >> 3;
}
inline i32 get_file(const i32 sq) {
	return sq & 7;
}
inline i32 get_sq(const i32 rank, const i32 file) {
	return (rank << 3) + file;
}
inline i32 mirror_sq(const i32 sq) {
	return sq ^ 56;
}
inline Color flip_col(const Color col) {
	return static_cast<Color>(col ^ 1);
}